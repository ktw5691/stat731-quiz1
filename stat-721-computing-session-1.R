...
#  Exercise 1: (a) Redo the same to compute the probability that a fair die lands on 5
   ...
   n <- 10000; f <- 0
   for(i in 1:n){f <- f + ifelse(sample(1:6)[1]==5,1,0)}
   cat("\n Pr[5] = ", f/n, "\n")
...
#  Activity: (a) Change the length of the sequence using 4, 5, 7 , 10, 100
   sequence <- NULL
   for(i in 1:4) sequence <- c(sequence, sample(c('H','T'),1))
   print(sequence)
   ...
   sequence <- NULL
   for(i in 1:7) sequence <- c(sequence, sample(c('H','T'),1))
   print(sequence)
   
   sequence <- NULL
   for(i in 1:10) sequence <- c(sequence, sample(c('H','T'),1))
   print(sequence)
   
   sequence <- NULL
   for(i in 1:100) sequence <- c(sequence, sample(c('H','T'),1))
   print(sequence)
...
#  (a) The first yields an outcome greater than the second
   ...
   n <- 100000
   f <- 0
   for(i in 1:n) f <- f + ifelse((sample(6)[1]>sample(6)[1]),1,0)
   print(f/n)
...
#  (b) The sum of the outcomes is less than 9
   ...
   n <- 100000
   f <- 0
   for(i in 1:n) f <- f + ifelse((sum(sample(6)[1]+sample(6)[1])<9),1,0)
   print(f/n)
   
#  (c) In each case above compare your empircal probabilities to the theoretical counterparts

#  Exercise 3:  Toss three coins and compute empirical probability that
#  (a) Getting at least one head
   
   set.seed(8262009)
   n <- 100000
   f <- 0
   for(i in 1:n) f <- f + ifelse(paste0(sample(c('H','T'))[1],sample(c('H','T'))[1],sample(c('H','T'))[1])%in%c('TTT'),0,1)   
   cat('\n Pr[Head] = ',f/n,'\n')
...
#   Allowing repetitions through iterations
    ...
    f <- 0
    for(i in 1:n)
    f <- f + ifelse(sample(c(-2,0,1,3),1)==-2,1,0)  # Defining our random variable (function!!!)
    print(f/n)    
...
#  Combinatorial Enumeration
   ...
   { 
     newword <- ''
     for(j in 1:2){newword <- paste(newword, letters[w2[i,j]],sep='')}
   ...
   { 
     newword <- ''
     for(j in 1:3){newword <- paste(newword, letters[w3[i,j]],sep='')}
     dictionary <- c(dictionary,newword)
   }
   ...
   { 
     newword <- ''
     for(j in 1:4){newword <- paste(newword, letters[w4[i,j]],sep='')}
     dictionary <- c(dictionary,newword)
   }
...
#   (a) Letters cannot be repeated and order does not matter
   set.seed(8262009)
   nw.2 <- nsamp(n=5, k=2, replace=F, ordered=F)
   nw.3 <- nsamp(n=5, k=3, replace=F, ordered=F)
   nw.4 <- nsamp(n=5, k=4, replace=F, ordered=F)
   ds <- nw.2 + nw.3 + nw.4
   
   A  <- c('a','b','c','d','e')
   dictionary <- NULL
   w2 <- urnsamples(A, size=2, replace=F, ordered=F)
   for(i in 1:nrow(w2)) { 
           newword <- ''
           for(j in 1:2){newword <- paste(newword, as.character(w2[i,j]),sep='')}
           dictionary <- c(dictionary,newword)
   }   
   w3 <- urnsamples(A, size=3, replace=F, ordered=F)
   for(i in 1:nrow(w3)) 
   { 
           newword <- ''
   ...
   w4 <- urnsamples(A, size=4, replace=F, ordered=F)
   for(i in 1:nrow(w4)) { 
           newword <- ''
...
#   (b) Letters cannot be repeated but order matters
   ...
   set.seed(8262009)
   nw.2 <- nsamp(n=5, k=2, replace=F, ordered=T)
   nw.3 <- nsamp(n=5, k=3, replace=F, ordered=T)
   nw.4 <- nsamp(n=5, k=4, replace=F, ordered=T)
   ds <- nw.2 + nw.3 + nw.4
   
   A  <- c('a','b','c','d','e')
   dictionary <- NULL
   w2 <- urnsamples(A, size=2, replace=F, ordered=T)
   for(i in 1:nrow(w2)) { 
           newword <- ''
           for(j in 1:2){newword <- paste(newword, as.character(w2[i,j]),sep='')}
           dictionary <- c(dictionary,newword)
   }   
   w3 <- urnsamples(A, size=3, replace=F, ordered=T)
   for(i in 1:nrow(w3)) 
   { 
           newword <- ''
           for(j in 1:3){newword <- paste(newword, as.character(w3[i,j]),sep='')}
   ...
   w4 <- urnsamples(A, size=4, replace=F, ordered=T)
   for(i in 1:nrow(w4)) { 
           newword <- ''
           for(j in 1:4){newword <- paste(newword, as.character(w4[i,j]),sep='')}
           dictionary <- c(dictionary,newword)
...
#   (c) Letter can be repeated and order matters
   ...
   set.seed(8262009)
   nw.2 <- nsamp(n=5, k=2, replace=T, ordered=T)
   nw.3 <- nsamp(n=5, k=3, replace=T, ordered=T)
   nw.4 <- nsamp(n=5, k=4, replace=T, ordered=T)
   ds <- nw.2 + nw.3 + nw.4
   
   A  <- c('a','b','c','d','e')
   dictionary <- NULL
   w2 <- urnsamples(A, size=2, replace=T, ordered=T)
   for(i in 1:nrow(w2)) { 
           newword <- ''
           for(j in 1:2){newword <- paste(newword, as.character(w2[i,j]),sep='')}
           dictionary <- c(dictionary,newword)
   }   
   w3 <- urnsamples(A, size=3, replace=T, ordered=T)
   for(i in 1:nrow(w3)) 
   { 
           newword <- ''
           for(j in 1:3){newword <- paste(newword, as.character(w3[i,j]),sep='')}
   ...
   w4 <- urnsamples(A, size=4, replace=T, ordered=T)
   for(i in 1:nrow(w4)) { 
           newword <- ''
           for(j in 1:4){newword <- paste(newword, as.character(w4[i,j]),sep='')}
           dictionary <- c(dictionary,newword)
...
#  (b) Create the probability space
   ...
   # Check to see if pmf is a valid pmf and sums to 1
   # Check to see if pmf is a valid pmf and is non-negative
   print(sum(f(S)))
   print(f(S)>0)
      
#  (c) Generate a barplot of the pmf of X 

   barplot(f(S), names=c('x=-2','x=0','x=1','x=3'),col='blue', ylab='Pr[X=x]', ylim = c(0,0.6))
...
#
   ...
   n <- 10000
   x<-runif(n, 1/5,3/5)
   probE.MC <- ((3/5-1/5)/n)*sum(3*(x^2))
   cat('\n Monte Carlo Pr[1/5 < X < 3/5] =',probE.MC,'\n')
...
#  (b) Find Pr[A] = Pr[X>1]
   ...
   f <- function(x,y){return(0.25*x*y)}
   n   <- 10000000
   a11 <- 1; a12 <- 2; a21 <- 0; a22 <- 2
   x   <- runif(n,a11,a12)
   y   <- runif(n,a21,a22)
   pxy <- 1/4*(x*y)
   
   pA.MC <- ((a12-a11)*(a22-a21)/n)*sum(f(x,y))
   cat('\n Pr[A] = ', pA.MC, '\n') 
      
#  (c) Find Pr[B] = Pr[Y<1]  
   a11 <- 0; a12 <- 2; a21 <- 0; a22 <- 1
   x   <- runif(n,a11,a12)
   y   <- runif(n,a21,a22)
   pxy <- 1/4*(x*y)
   
   pB.MC  <- ((a12-a11)*(a22-a21)/n)*sum(f(x,y))
   cat('\n Pr[B] = ', pB.MC, '\n') 
#  (d) Find Pr[A and B] = Pr[X > 1 and Y < 1]
   ...
   y   <- runif(n,a21,a22)
   pxy <- 1/4*(x*y)
   pAB.MC <- ((a12-a11)*(a22-a21)/n)*sum(f(x,y))
   cat('\n Pr[A and B] = ', pAB.MC, '\n') 
